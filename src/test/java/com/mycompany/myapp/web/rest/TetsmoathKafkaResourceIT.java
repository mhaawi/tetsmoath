...
class TetsmoathKafkaResourceIT {
    ...
    void setup() {
        ...
        TetsmoathKafkaResource kafkaResource = new TetsmoathKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/tetsmoath-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/tetsmoath-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
